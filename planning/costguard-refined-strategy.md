# CostGuard: Refined Product Strategy (Based on PyCostAudit Learnings)
...
**What CostGuard adds:** Automatic cost-spike interception at runtime (not just tracking)
...
- **Problem:** No compression, no rollingsummarization
...
### Python SDK
```python
from costguard import CostGuardRuntime, CostMeter
...
response = guard.run_agent_task(
    ...
    context={"file": "src/auth.rs"}  # CostGuard auto-handles PDF extraction, etc.
)
...
### Node.js SDK
```javascript
const { CostGuardRuntime } = require("costguard");

const guard = new CostGuardRuntime({
  model: "claude-3-5-sonnet",
...
## The Three Pillars of CostGuard
...
| Feature | Claude Code | Deep Agents | Hermes | CostGuard |
|---------|-------------|------------|--------|-----------|
...
**For Claude Code users:** "CostGuard pays for itself in 2 weeks."
...
**Why Rust for CostGuard (vs PyCostAudit's Rust):**
...
**CostGuard is NOT a replacement for PyCostAudit.**

Instead:
- **PyCostAudit:** Historical cost tracking & reporting (analytics)
- **CostGuard:** Real-time cost interception & optimization (runtime)

**Integration:**
- CostGuard's cost calculations feed into PyCostAudit's reporting
- Users can track both: actual spend (PyCostAudit) + what-ifs (CostGuard)
