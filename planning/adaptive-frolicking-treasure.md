...
### Gap
`EpisodeScorer` computes per-episode scores within one dataset. No cross-dataset comparison, percentile ranking, or mixing-ratio recommendation.
...
**`tests/test_hdf5.py`** / **`tests/test_netcdf.py`** (new): `pytest.importorskip`; build minimal fixture with 2 episodes; verify `inspect()`, `to_robotics_dataframe()`, `convert_*()` output schema.
...
**`tests/test_rlds.py`** (new): `pytest.importorskip("tensorflow_datasets")`; minimal TFRecord fixture (2 episodes × 10 steps); verify `convert_rlds` output Parquet schema and episode count.
...
**`python/pyroboframes/distributed.py`** — extend (read existing first, preserve what's there):
...
class RemoteDataset:
    ...
    def loader(self, **kwargs) -> DataLoader: ...   # streams/caches on demand
...
5. **HDF5/NetCDF** — pure Python; no Rust
